...
# -*- coding: utf-8 -*-
"""
通过火山方舟「数据获取」chat/completions 生成侨乡文化 JSON，写入 date/culture_ark.json。
...
运行（项目根目录）：
  py -3 scripts/fetch_culture_data_api.py
...
def call_ark(
    ...
    *,
    ...
    user = """请整理广西玉林的侨乡文化信息，包括历史时间线、侨胞故事等。

严格输出一个 JSON 对象，格式如下（不要其它文字）：
{
  "history": [
    {"year":"年份","event":"历史事件","image":"图片路径"}
  ],
  "figures": [
    {"name":"姓名","title":"身份/头衔","story":"故事简介","tag":"标签（侨商风采/侨胞故事）","image":"图片路径"}
  ],
  "stories": [
    {"title":"标题","role":"角色","period":"时期","story":"故事内容","highlight":"亮点","image":"图片路径"}
  ]
}

要求：
- history 含 6～10 个历史时间节点；
- figures 含 4～6 个侨胞人物；
- stories 含 4～6 个文化故事；
- image 为图片路径，使用相对路径如 "images/placeholders/carousel-1.svg"。""" + _output_lang_instruction()
...
def main() -> int:
    ...
    out_path = Path(os.environ.get("OUT", str(ROOT / "date" / "culture_ark.json"))).resolve()
    ...
    print(f"已写入 {out_path} ，包含历史、人物、故事数据", file=sys.stderr)
    return 0
